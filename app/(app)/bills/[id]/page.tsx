...
export default function BillDetailsPage({ params }: BillDetailsPageProps) {
  ...
  const initialBill = useMemo(() => data?.find((bill) => bill.id === id), [data, id])
      ...
      <section className="space-y-4 rounded-3xl border border-border/60 bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs uppercase tracking-widest text-muted-foreground">You {bill.youOwe >= 0 ? 'owe' : 'get back'}</p>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <GhostButton onClick={() => toast.info('Receipt coming soon!')}>
            View receipt
          </GhostButton>
        </div>
      </section>
